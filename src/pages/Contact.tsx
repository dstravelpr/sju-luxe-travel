...
const Contact = () => {
  ...
  return (
    <Layout>
      <SEOHead
        title="Free Travel Consultation | SJU Luxe Travel"
        description="Schedule a free bespoke travel consultation with your boutique agent in San Juan, PR. Private tours, cruises & bespoke itineraries with NCM Concierge perks."
        titleEs="Consulta Gratis con tu Agente de Viajes | SJU Luxe Travel"
        descriptionEs="Agenda una consulta gratis con tu agente de viajes en San Juan, PR. Tours privados, cruceros e itinerarios a medida con beneficios NCM Concierge."
        breadcrumbs={[
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-12 max-w-5xl mx-auto">
            <div>
              <h2 className="font-heading text-2xl text-foreground mb-8">
                ...
                <span className="text-gold-gradient italic">
                  {t.contact.sidebarTitleHighlight}
                </span>
              </h2>
              ...
              {sent ? (
                ...
                >
                  <input
                    type="hidden"
                    name="_subject"
                    value="New consultation request from SJU Luxe Travel"
                  />
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div>
                      <label className="text-foreground text-xs mb-1.5 block">
                        {t.contact.travelType}
                      </label>
                      <select
                        name="travelType"
                        className="w-full px-4 py-3 bg-charcoal gold-border rounded-sm text-muted-foreground text-sm focus:outline-none focus:border-gold transition-colors appearance-none"
                      >
                        <option value="">{t.contact.travelTypeSelect}</option>
                        <option value="honeymoon">
                          {t.contact.travelTypes.honeymoon}
                        </option>
                        <option value="anniversary">
                          {t.contact.travelTypes.anniversary}
                        </option>
                        <option value="family">
                          {t.contact.travelTypes.family}
                        </option>
                        <option value="adventure">
                          {t.contact.travelTypes.adventure}
                        </option>
                        <option value="wellness">
                          {t.contact.travelTypes.wellness}
                        </option>
                        <option value="cruise">
                          {t.contact.travelTypes.cruise}
                        </option>
                        <option value="soloWomen">
                          {t.contact.travelTypes.soloWomen}
                        </option>
                        <option value="incentive">
                          {t.contact.travelTypes.incentive}
                        </option>
                        <option value="other">
                          {t.contact.travelTypes.other}
                        </option>
                      </select>
                    </div>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                      <label className="text-foreground text-xs mb-1.5 block">
                        {t.contact.desiredDestinations}
                      </label>
                      <input
                        type="text"
                        name="destinations"
                        placeholder={t.contact.destinationsPlaceholder}
                        className="w-full px-4 py-3 bg-charcoal gold-border rounded-sm text-foreground text-sm placeholder:text-muted-foreground focus:outline-none focus:border-gold transition-colors"
                      />
                    </div>
                    <div>
                      <label className="text-foreground text-xs mb-1.5 block">
                        {t.contact.preferredDates}
                      </label>
                      <input
                        type="text"
                        name="dates"
                        placeholder={t.contact.datesPlaceholder}
                        className="w-full px-4 py-3 bg-charcoal gold-border rounded-sm text-foreground text-sm placeholder:text-muted-foreground focus:outline-none focus:border-gold transition-colors"
                      />
                    </div>
                  </div>
                  <div>
                    <label className="text-foreground text-xs mb-1.5 block">
                      {t.contact.dreamTrip} <span className="text-gold">*</span>
                    </label>
                    <textarea
                      name="message"
                      rows={5}
                      placeholder={t.contact.dreamTripPlaceholder}
                      required
                  ...
                  <button
                    type="submit"
                    disabled={submitting}
                    className="inline-flex items-center gap-2 px-8 py-3 bg-gold-gradient text-primary-foreground font-body text-xs font-semibold tracking-[0.15em] uppercase rounded-sm transition-all duration-300 hover:shadow-[0_0_30px_-5px_hsl(43,74%,49%,0.5)] hover:scale-105 disabled:opacity-60 disabled:pointer-events-none"
                  >
      ...
      <section className="py-16 bg-background text-center">
        <div className="container mx-auto px-6">
          ...
          <h3 className="font-heading text-2xl text-foreground mb-2">
            San <span className="text-gold-gradient italic">Juan</span>, Puerto
            Rico
          </h3>
          <p className="text-muted-foreground text-sm">
            {t.contact.locationBanner}
          </p>
        </div>
      </section>
    </Layout>
